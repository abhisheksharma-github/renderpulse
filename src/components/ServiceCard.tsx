  ...
  return (
    <div 
      className={`glass-panel rounded-2xl p-4 flex flex-col justify-between transition-all duration-300 relative border ${
        service.selected 
          ? 'border-emerald-500/40 bg-slate-900/85 shadow-lg shadow-emerald-500/5' 
    ...
    >
      ...
      <div>
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <div className="flex items-start gap-2.5 min-w-0">
            <input
              ...
              className="mt-1 h-4 w-4 rounded bg-slate-900 border-slate-700 text-emerald-500 focus:ring-emerald-500/30 focus:ring-offset-0 cursor-pointer accent-emerald-500"
            />
            <div className="min-w-0">
              <h3 className="text-sm font-bold text-white tracking-tight truncate flex items-center gap-1.5" title={service.name}>
                {service.name}
              </h3>
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-slate-800/90 text-slate-300 border border-slate-700/60">
        ...
        {/* Target URL Capsule */}
        <div className="rounded-xl bg-slate-950/80 p-2.5 border border-slate-800/80 mb-3 space-y-1.5">
          <div className="flex items-center justify-between gap-2 text-xs">
            <a
              href={service.url}
              target="_blank"
              rel="noopener noreferrer"
              className="text-slate-300 hover:text-emerald-400 font-mono text-[11px] truncate flex items-center gap-1 group transition-colors"
              title={`Open ${service.url}`}
            >
              <span className="truncate">{service.url}</span>
              <ExternalLink className="h-2.5 w-2.5 text-slate-500 group-hover:text-emerald-400 shrink-0" />
            </a>
            <button
        ...
        {service.hasRobotsWarning && (
          <div className="mb-3 rounded-lg bg-amber-950/30 border border-amber-500/30 p-2 text-[10px] text-amber-300 flex items-start gap-1.5">
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          {/* Status Badge */}
          {isSleeping && (
            <div className="flex items-center gap-1.5 text-xs font-semibold text-amber-400 bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/20">
          ...
          {isWaking && (
            <div className="flex items-center gap-2 text-xs font-semibold text-indigo-300 bg-indigo-500/15 px-2.5 py-1 rounded-full border border-indigo-500/30 animate-pulse">
          ...
          <div className="text-[11px] font-mono text-slate-400 flex items-center gap-1">
            {service.lastLatency ? (
              <span className="text-emerald-400 font-semibold">{service.lastLatency}ms</span>
        ...
        {isWaking && (
          <div className="space-y-1 mt-2">
            <div className="h-2 w-full overflow-hidden rounded-full bg-slate-900 border border-indigo-900/40">
              <div 
        ...
        {isFailed && service.errorMsg && (
          <p className="text-[11px] text-rose-300 font-mono mt-1 bg-rose-950/20 p-1.5 rounded-lg border border-rose-900/30">
            {service.errorMsg}
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800/80">
        ...
        <button
          ...
          disabled={isWaking}
          className="flex items-center justify-center gap-1.5 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 active:scale-95 text-emerald-300 hover:text-emerald-200 border border-emerald-500/30 py-2.5 text-xs font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
